...
function getAllResources($db) {
    ...
    $sort = $_GET["sort"] ?? null;
    $validSortFields = ['title', 'created_at'];
    ...
    $order = in_array($order, $validSortOrders) ? $order : 'desc';
...
function createResource($db, $data) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        sendResponse(['success' => true, 'message' => 'Resource created successfully', 'id' => $newResourceId], 201);
    } else {
...
function updateResource($db, $data) {
    ...
    $stmt->bindValue(count($values) + 1,$data["id"]);
    // TODO: Execute the query
    $stmt->execute();
    ...
    if ($stmt->rowCount() > 0) {
        ...
        sendResponse(['success' => false, 'message' => 'Failed to update resource or no changes made'], 500);
    }
}
...
function deleteResource($db, $resourceId) {
    ...
    try {
        ...
        $sql = "DELETE FROM comments WHERE resource_id = ?";
        $stmt = $db->prepare($sql);
        ...
        sendResponse(['success' => true, 'message' => "Resource deleted (ID: {$resourceId})"], 200);
    } catch (Exception $e) {
...
function createComment($db, $data) {
    ...
    $sql = "SELECT id FROM resources WHERE id = ?";
    $stmt = $db->prepare($sql);
    $stmt->bindValue(1, $data->resource_id);
    $stmt->execute();
    $resource = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$resource) {
        sendResponse(['success' => false, 'message' => 'Resource not found'], 404);
    ...
    if ($stmt->rowCount() > 0) {
        ...
        sendResponse(['success' => true, 'message' => 'Comment created', 'id' => $commentId], 201);
    } else {
...
try {
    ...
    if ($method === 'GET') {
        ...
        if ($action === 'comment') {
            ...
            createResource($db, $requestBody);
        }
        
    } elseif ($method === 'PUT') {
        // TODO: Update a resource
        // Call updateResource()
        updateResource($db, $requestBody);
    ...
    sendResponse(['success' => false, 'message' => 'Internal server error occurred'], 500);
}
